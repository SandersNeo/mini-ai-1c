...
<!-- news:start -->
> ⚠️ **Qwen Code CLI стал платным** (апрель 2025). Бесплатный доступ через OAuth более не работает — токены отклоняются с ошибкой 401.
...
<!-- release-news:start -->
> **📋 Последний релиз: версия 1.2.3** — 21 апреля 2026
>
> - **Интеграция с Конфигуратором 1С стала стабильной** — `/описание` и `/доработай` через Ctrl+ПКМ вставляют код именно в ту функцию, на которой был клик, без «моргания» всего модуля
> - **Правая кнопка мыши не «залипает»** — контекстное меню Конфигуратора корректно подавляется на паре DOWN/UP + таймаут-автосброс 1.5 с
> - **Локальные модели снова отвечают** — Ollama / LM Studio перестали задавать лишние уточняющие вопросы, корректный `context_window` читается прямо с сервера
> - **Версия видна везде** — в заголовке окна, настройках и уведомлениях об обновлении, убрана призрачная иконка в трее после закрытия
>
> [Подробнее о релизе →](https://github.com/hawkxtreme/mini-ai-1c/releases/tag/v1.2.3) · [Все релизы →](https://github.com/hawkxtreme/mini-ai-1c/releases)
<!-- release-news:end -->
